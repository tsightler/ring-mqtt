...
export default new class Mqtt {
    ...
    // The MQTT client throws on any topic which isn't a non-empty string.  As these events are
    // emitted from async device code, that error surfaces as an unhandled promise rejection
    // which can take down the process, so contain a bad topic here instead.
    isValidTopic(topic, action) {
        if (typeof topic === 'string' && topic.length > 0) { return true }
        debug(chalk.yellow(`Ignoring attempt to ${action} invalid MQTT topic: ${topic}`))
        return false
    }

    async initBroker() {
        // Start internal broker, used only for inter-process communication (IPC)
        const mqttServer = await Aedes.createBroker()
