...
export function recordAndUpload(streamUrl) {
    debug(`Recording function called`)
    const lastSlashIndex = streamUrl.lastIndexOf("/");
    debug(`slasher: ${lastSlashIndex}`)
    const deviceId = streamUrl.substring(lastSlashIndex + 1);
    debug(`deviceId: ${lastSlashIndex}`)
    const liveStreamUrl = `rtsp://${utils.config().livestream_user}:${utils.config().livestream_pass}@${utils.getHostIp()}:8554/${deviceId}`;
    debug(`streamUrl: ${liveStreamUrl}`)
