...
class Config {
    ...
    doAutoConfig() {
        if (!this.data.mqtt_url) {
            ...
            this.data.mqtt_user = this.data.mqtt_user === '<auto_detect>' ? '[auto_port]' : this.data.mqtt_user
        }
        let mqttURL = this.data.mqtt_url
            ? (this.data.mqtt_url)
            : (`mqtt://${this.data.mqtt_user}:${this.data.mqtt_password}@${this.data.mqtt_host}:${this.data.mqtt_port}`)
        try {
