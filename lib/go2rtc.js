        ...
        if (cameras) {
            ...
            for (const camera of cameras) {
                this.config.streams[`${camera.deviceId}_live`] =  
                    `exec:./scripts/start-stream.sh ${camera.deviceId} live ${camera.deviceTopic} {output}`
                this.config.streams[`${camera.deviceId}_event`] =  
                    `exec:./scripts/start-stream.sh ${camera.deviceId} event ${camera.deviceTopic} {output}`
            }
            try {
