...
function processExit(exitCode) {
    if (streamState = 'ON') {
        mqtt.publish(commandTopic, 'OFF', { qos: 1 })
    }
...
mqttClient.on('connect', async function () {
    ...
    mqtt.subscribe(stateTopic, { qos: 1 })
    mqtt.publish(commandTopic, 'ON', { qos: 1 })
})
