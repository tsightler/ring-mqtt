...
class RtspSimpleServer {
    ...
    async start(cameras) {
        ...
        this.rssProcess.on('close', async (code) => {
            await utils.sleep(1) // Give a second to avoid spurious messages if shutting down
    ...
    async createAllRtspPaths() {
        ...
        for (const camera of this.cameras) {
            ...
            let rtspPathConfig = {
                ...
                runOnDemandStartTimeout: 5000000000,
            ...
            rtspPathConfig = {
                source: 'publisher',
                runOnDemand: `./scripts/start-stream.sh "${camera.deviceId}_event" "${camera.deviceData.name}" "${camera.deviceTopic}/event_stream/attributes" "${camera.deviceTopic}/event_stream/command"`,
                runOnDemandRestart: false,
                runOnDemandStartTimeout: 5000000000,
                runOnDemandCloseAfter: 10000000000,
                ...(camera.config.livestream_user && camera.config.livestream_pass) ? {
