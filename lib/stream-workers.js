...
export default new class StreamWorkers {
    constructor() {
        this.streamWorkers = []
    }
    ...
    init() {
        ...
        utils.event.on('start_livestream', (streamData) => {
            if (this.getWorkerId(streamData.deviceId) < 0) {
                ...
                utils.debug(chalk.green(`[${streamData.deviceName}] `)+`Live stream assigned to worker ${parseInt(workerId)+1} with ${fewestSessions} current active ${fewestSessions === 1 ? 'session' : 'sessions'}`)
        ...
        utils.event.on('stop_livestream', (streamData) => {
            ...
            if (workerId >= 0) {
                utils.debug(chalk.green(`[${streamData.deviceName}] `)+`Stopping live stream session on workerId ${parseInt(workerId)+1}`)
                ...
                utils.debug(chalk.green(`[${streamData.deviceName}] `)+'Received live stream stop command but no active session was found')
