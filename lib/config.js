...
class Config {
    ...
    async migrateAddonConfig() {
        ...
        debug(headers)
        const addonConfig = await got(`http://supervisor/addons/self/options/config`, options).json()
        debug(addonConfig)
    }
}
