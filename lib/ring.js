...
class RingMqtt {
    ...
    async updateRingData() {
        ...
        debug(colors.green(' '.repeat(80)))
        debug(colors.brightYellow('IMPORTANT: ')+colors.white('All alarm and smart lighting hubs at the locations listed above      '))
        debug(colors.white('           *MUST* be online or device discovery for that location will hang!    '))
        debug(colors.white('           If desired, the "location_ids" config option can be use to restrict  '))
        debug(colors.white('           discovery to specific locations only. See documentation for details. '))
