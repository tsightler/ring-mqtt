...
class State {
    ...
    async init(config) {
        ...
        utils.event.on(`update_device_state`, (deviceId, stateData) => {
        ...
        utils.event.on('get_bypass_mode_states', () => {
            const bypassModeStates = {}
            Object.keys(this.data.devices).map(key => {
                if (this.data.devices[key].hasOwnProperty('bypass_mode')) {
                    bypassModeStates[key] = this.data.devices[key].bypass_mode
                }
            })
            utils.event.emit('bypass_mode_states', bypassModeStates)
        })
    }
