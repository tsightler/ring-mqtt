...
class Go2RTC {
    constructor() {
        ...
        this.candidates = (process.env.HAIPADDRESS) ? process.env.HAIPADDRESS : 'stun'
        this.config = {
            log: {
                level: "debug"
            },
    ...
    async init(cameras) {
        ...
        if (cameras) {
            ...
            for (const camera of cameras) {
                this.config.streams[`${camera.deviceId}_live`] =  
                    `exec:${require('path').dirname(require.main.filename)}/scripts/start-stream.sh ${camera.deviceId} live ${camera.deviceTopic} {output}`
                this.config.streams[`${camera.deviceId}_event`] =  
                    `exec:${require('path').dirname(require.main.filename)}/scripts/start-stream.sh ${camera.deviceId} event ${camera.deviceTopic} {output}`
            }
            try {
        ...
        stdoutLine.on('line', (line) => {
            ...
            debug(line.replace(/^.*\d{2}:\d{2}:\d{2}\.\d{3}([^\s]+) /, colors.green('[go2rtc] ')))
        })
