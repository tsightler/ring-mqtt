...
export default new class Go2RTC {
    ...
    async init(cameras) {
        ...
        const candidates = []
        const hostIpAddress = await utils.getHostIp()

        if (hostIpAddress !== 'localhost') {
            candidates.push(hostIpAddress+":8555")
        }
        candidates.push('stun:8555')
