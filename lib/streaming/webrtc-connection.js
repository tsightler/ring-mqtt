...
export class WebrtcConnection extends Subscribed {
    constructor(ticket, camera, options = {}) {
        ...
        const pc = new WeriftPeerConnection(options)
        this.pc = pc
        this.onAudioRtp = pc.onAudioRtp
        this.onVideoRtp = pc.onVideoRtp
        this.onAudioRtcp = pc.onAudioRtcp
        this.onVideoRtcp = pc.onVideoRtcp
        this.onVideoCodec = pc.onVideoCodec
        this.onReceivedVideoCodec = pc.onReceivedVideoCodec
        ...
        this.addSubscriptions(
            onMessage.pipe(concatMap((event) => {
                ...
                // Without this an error escapes the pipeline as an unhandled rejection,
                // which in a worker thread terminates the entire worker
                error: (error) => {
                    parentPort.postMessage({type: 'log_error', data: `Error processing signaling message: ${error.message}`})
                    this.callEnded()
                }
            }),
            ...
            onClose.subscribe((event) => {
                this.log(`Websocket signaling connection closed${event?.code ? ` with code ${event.code}${event.reason ? ` (${event.reason})` : ''}` : ''}`)
                this.callEnded()
            }),
        ...
        switch (message.method) {
            ...
            case 'sdp':
                try {
                    ...
                    // Ring answered with nothing this offer can use, which werift reports
                    // as "negotiate codecs failed".  Flagged rather than thrown so that the
                    // caller can retry with a different codec instead of the error escaping
                    // into the message pipeline and taking the whole worker down with it.
                    this.answerFailed = true
                    this.log(`Ring's answer could not be used: ${error.message}`)
                    this.callEnded()
                    return
                }
            ...
            case 'pong':
                return
            case 'timed_metadata':
                // Bird's eye view cameras send location metadata which is currently ignored.
                return
            case 'camera_started':
                this.log('Ring reports the camera has started streaming')
                return
            case 'notification': {
            ...
            case 'close':
                this.log(`Ring signaling ended the session: ${JSON.stringify(message.body)}`)
                this.callEnded()
                return
        }

        // Anything reaching here is a message this code does not understand, which is
        // worth seeing since it may explain a session ending for no apparent reason
        this.log(`Unhandled signaling message: ${JSON.stringify(message)}`)
    }
