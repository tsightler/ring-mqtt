...
class Config {
    constructor() {
        ...
        debug(`MQTT URL: ${mqttURL.protocol}//${mqttURL.username ? mqttURL.username+':' : ''}${mqttURL.password ? mqttURL.password+'@' : ''}${mqttURL.hostname}:${mqttURL.port}`)
    }
    ...
    doAutoConfig() {
        ...
        // Remove prior to publish of v5.x
        if (!this.data.mqtt_url) {
