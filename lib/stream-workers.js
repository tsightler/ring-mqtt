...
// a single core and keep the main NodeJS thread busy and strugling to keep up with
...
class StreamWorkers {
    ...
    init() {
        ...
        utils.event.on('stop_livestream', (streamData) => {
            ...
            if (workerId >= 0) {
                utils.debug(colors.green(`[${streamData.deviceName}] `)+`Received a request to stop live stream session for ${streamData.deviceId}, on workerId ${workerId}`)
                ...
                utils.debug(colors.green(`[${streamData.deviceName}] `)+'Received a request to stop live stream but no active stream session was found')
            }
        })
    }
