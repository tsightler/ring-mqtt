...
export default new class RingMqtt {
    ...
    async init(state, generatedToken) {
        ...
        if (this.client) {
            ...
            const ringAuth = {
                ...
                ...state.data?.push_credentials ? { pushCredentials: state.data.push_credentials } :  {}
            }
    ...
    async updatePushToken() {
        if (this.client.restClient.authOptions?.pushCredentials) {
            debug('Updating FCM push token via Ring API')
            debug({
                ...
                json: {
                    device: {
                    metadata: {
                        ...this.client.restClient.baseSessionMetadata,
                        pn_service: 'fcm',
                    },
                    os: 'android',
                    push_notification_token: this.client.restClient.authOptions.pushCredentials.fcm.token,
                    },
                },
            })
            try {
                await this.client.restClient.request({
                    url: 'https://api.ring.com/clients_api/device',
                    method: 'PATCH',
                    json: {
                        device: {
