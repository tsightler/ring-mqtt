...
class Thermostat extends RingSocketDevice {
    ...
    async publishData() {
        ...
        this.publishMqtt(this.entities.climate.temperature_state_topic, this.device.data.setPoint ? this.device.data.setPoint.toString() : null, true)
        this.publishMqtt(this.entities.climate.fan_mode_state_topic, this.device.data.fanMode.replace(/^./, str => str.toUpperCase()), true)
        if (this.operatingStatus) {
