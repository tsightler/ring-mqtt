...
// to straight Javascript with some code not required for ring-mqtt removed.
// Much thanks to @dgreif for the original code which is the basis of this work.
...
export class StreamingSession extends Subscribed {
    ...
    bindToConnection(connection) {
        this.addSubscriptions(connection.onAudioRtp.subscribe(this.onAudioRtp), 
    ...
    async startTranscoding(ffmpegOptions) {
        ...
        this.onAudioRtp.pipe((rtp) => {
            this.audioSplitter.send(rtp.serialize(), { port: audioPort })
        })

        this.onVideoRtp.pipe((rtp) => {
                this.videoSplitter.send(rtp.serialize(), { port: videoPort })
        })
