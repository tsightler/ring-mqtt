...
export default new class TokenApp {
    ...
    async start() {
        ...
        this.app.get('/', (req, res) => {
            res.cookie('systemId', `${process.env.RUNMODE === 'addon' ? 'ring-mqtt-addon' : 'ring-mqtt'}-${this.systemId.slice(-5)}`, { maxAge: 1000, encode: String })
            ...
            try {
                ...
                if (restClient.using2fa) {
                    ...
                    res.cookie('error', error.message, { maxAge: 1000, encode: String })
                       .cookie('systemId', `${process.env.RUNMODE === 'addon' ? 'ring-mqtt-addon' : 'ring-mqtt'}-${this.systemId.slice(-5)}`, { maxAge: 1000, encode: String })
                       .sendFile('account.html', {root: webdir})
                }
            }
        })
