...
class StreamWorkers {
    ...
    init() {
        ...
        utils.event.on('start_livestream', (deviceId, deviceName, sessionId, rtspPublishUrl) => {
            ...
            utils.debug(`Allocated requested live stream to worker ${parseInt(workerId)+1} with ${Object.keys(this.streamWorkers[workerId].sessions).length} active sessions`)
