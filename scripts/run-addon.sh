...
if [ "${BRANCH}" = "latest" ]; then
    DEBUG=ring-mqtt HASSADDON=true NODE_OPTIONS="--unhandled-rejections=warn" exec /app/ring-mqtt-latest/ring-mqtt.js
elif [ "${BRANCH}" = "dev" ]; then
    DEBUG=ring-mqtt HASSADDON=true NODE_OPTIONS="--unhandled-rejections=warn" exec /app/ring-mqtt-dev/ring-mqtt.js
else
    DEBUG=ring-mqtt HASSADDON=true NODE_OPTIONS="--unhandled-rejections=warn" exec /app/ring-mqtt/ring-mqtt.js
fi
