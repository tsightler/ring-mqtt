...
const h265Codec = () => new RTCRtpCodecParameters({
    ...
    payloadType: 100,
    rtcpFeedback: videoRtcpFeedback(),
    // level-id is the HEVC level multiplied by 30, so the 93 which appears in most WebRTC
    // examples means level 3.1, which tops out around 1280x720.  Ring sends far larger
    // pictures than that, 2688x1520 among them, and a decoder set up from a level which
    // does not cover the picture it is given can initialise and then fail part way through.
    // Nothing here decodes video, so the honest thing to advertise is a level high enough
    // to cover anything Ring might send, which is what 186 (level 6.2) does.
    parameters: 'profile-id=1;level-id=186;tier-flag=0;tx-mode=SRST',
})
...
// Negotiating transport wide congestion control makes werift report back to Ring how well
// packets are arriving, which lets Ring's encoder adapt its bitrate.  That sounds desirable
// but it hands a remote encoder control of the bitrate based on werift's feedback, and that
// implementation is visibly incomplete: its periodic sender never runs, because twccRunning
// is initialised false before the loop which reads it, so reports only go out in bursts.
// Irregular feedback is worse than none, and the original code negotiated no extensions at
// all, so this stays off unless explicitly asked for.
const sendCongestionFeedback = process.env.RINGMQTT_TWCC === 'on'

const buildHeaderExtensions = () => ({
    video: sendCongestionFeedback
        ? [useSdesMid(), useTransportWideCC()]
        : [useSdesMid()],
})
