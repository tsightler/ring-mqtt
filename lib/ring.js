...
import {
    DEVICE_REPUBLISH_COUNT,
    HA_RESTART_REPUBLISH_DELAY_SEC,
    DEVICE_DISCOVERY_LOG_INTERVAL_MS,
    DEVICE_DISCOVERY_LOG_FREQUENCY,
    CAMERA_STATUS_POLLING_SEC,
    LOCATION_MODE_POLLING_SEC,
    WEBSOCKET_OFFLINE_DELAY_SEC,
    DEVICE_REPUBLISH_DELAY_SEC,
    CAMERA_EVENT_HISTORY_LIMIT
} from './constants.js'
...
export default new class RingMqtt {
    constructor() {
        ...
        this.republishCount = DEVICE_REPUBLISH_COUNT
        ...
        utils.event.on('ha_status', async (topic, message) => {
            ...
            if (message === 'online') {
                ...
                if (this.republishCount > 0) {
                    ...
                    debug(`Home Assistant restart detected, resending device config/state in ${HA_RESTART_REPUBLISH_DELAY_SEC} seconds`)
                    await utils.sleep(HA_RESTART_REPUBLISH_DELAY_SEC)
                    this.republishCount = DEVICE_REPUBLISH_COUNT
                    this.publishLocations()
                }
            }
    ...
    async init(state, generatedToken) {
        ...
        if (this.client) {
            try {
                ...
                this.client.restClient.refreshToken = this.refreshToken
                this.client.restClient._authPromise = undefined
                await utils.sleep(DEBUG_OUTPUT_DELAY_SEC)
            ...
            const ringAuth = {
                ...
                ...utils.config().enable_cameras ? { cameraStatusPollingSeconds: CAMERA_STATUS_POLLING_SEC } : {},
                ...utils.config().enable_modes ? { locationModePollingSeconds: LOCATION_MODE_POLLING_SEC } : {},
    ...
    async initRingData() {
        ...
        for (const location of locations) {
            ...
            const logInterval = setInterval(() => {
                ...
                if (suppressInterval % DEVICE_DISCOVERY_LOG_FREQUENCY === 0 && location.offlineAssets.length > 0) {
            ...
            if (cameras.length > 0) {
                ...
                try {
                    const response = await location.restClient.request({
                        method: 'GET',
                        url: `https://api.ring.com/evm/v2/history/devices?source_ids=${cameraIds}&capabilities=offline_event&limit=${CAMERA_EVENT_HISTORY_LIMIT}`
                    })
                ...
                if (location.hasHubs && !location.isSubscribed) {
                    ...
                    location.onConnected.subscribe(async connected => {
                        if (connected) {
                            ...
                            // Wait before setting devices offline in case disconnect is transient
                            // Keeps from creating "unknown" state for sensors if connection error is short lived
                            await utils.sleep(WEBSOCKET_OFFLINE_DELAY_SEC)
    ...
    async publishDevices(location) {
        ...
        while (this.republishCount > 0 && this.mqttConnected) {
            ...
            await utils.sleep(DEVICE_REPUBLISH_DELAY_SEC)
            this.republishCount--
        }
    }
