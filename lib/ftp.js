...
export function recordAndUpload(rtspPublishUrl) {
    ...
    debug(`DEBUG - ${rtspPublishUrl}`)
    const liveStreamUrl = `rtsp://${utils.config().livestream_user}:${utils.config().livestream_pass}@${streamIp}:8554/${utils.config().record_id}`;
